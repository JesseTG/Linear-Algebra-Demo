...
Duck::Duck()
{
    ...
    frames[DuckFrame::DOWN_1] = RectInt(98, 198, 132, 229);
    frames[DuckFrame::DOWN_2] = RectInt(136, 198, 170, 229);
    frames[DuckFrame::DOWN_3] = RectInt(173, 198, 206, 229);
    ...
    setRandomDirection();
    probChangeVel = .001;//Random::Random(0.0f, 1.0f);
}
...
void Duck::updateAnimation()
{
    ...
    if (animationtimer.GetElapsedTime() >= .125) {
        if (velocity.y >= 0) frame += 3;
        sprite.SetSubRect(frames[DuckFrame(++frame % 3)]);
